...
function delta(value: number | null | undefined) {
  if (value == null) return LAB_UNAVAILABLE;
...
export function LabIntegrityPanel() {
  ...
  const payload=radar.data as Awaited<ReturnType<typeof getShadowRadar>> | { stats?: any; cases?: any[]; rule?: string } | undefined;
  ...
  const variantReports=latestReplay?.report?.replay?.variants ?? [];
  const ranked=[...comparisons].sort((a,b)=>{
    const ae=a.evidenceLabel==="INSUFFICIENT";
    const be=b.evidenceLabel==="INSUFFICIENT";
    if(ae!==be) return ae ? 1 : -1;
    const ad=a.testDeltaVsBaselinePp ?? -Infinity;
    const bd=b.testDeltaVsBaselinePp ?? -Infinity;
    if(bd!==ad) return bd-ad;
    return (b.extraTestN??-1)-(a.extraTestN??-1);
  });
  return <section className="mt-2 space-y-3" data-lab-integrity>
    ...
    {q.isLoading?<p className="text-sm text-subtle">Leyendo métricas…</p>:q.isError?<p className="text-sm text-sell">No se ha podido leer el laboratorio. No se inventan cifras.</p>:<>
      ...
      <div className="overflow-hidden rounded-[var(--radius-lg)] bg-elevated shadow-[var(--shadow-border)]" data-shadow-comparator>
        <div className="border-b border-border px-4 py-3"><div className="flex items-center justify-between gap-3"><h3 className="font-semibold">Comparador Shadow</h3><span className="text-xs font-mono text-subtle">V1 + métodos alternativos</span></div><p className="mt-1 text-xs text-subtle">Misma muestra de episodios congelados. Ordenado por mejora en TEST frente a V1. No modifica V1.</p></div>
        {radar.isLoading?<p className="px-4 py-4 text-sm text-subtle">Calculando comparación…</p>:radar.isError||!shadowRadar?<p className="px-4 py-4 text-sm text-subtle">Comparador no disponible. No se inventan resultados.</p>:<>
          ...
          {ranked.length>0?<div className="divide-y divide-border">{ranked.map((c,i)=>{const vr=variantReports.find(x=>x.variant===c.variant);return <div key={c.variant} className="px-4 py-3"><div className="flex items-center justify-between gap-3"><div className="min-w-0"><div className="flex items-center gap-2 text-sm font-medium"><span className="font-mono text-xs text-subtle">#{i+1}</span><span className="truncate">{c.variant}</span>{c.variant==="BASELINE_V1"?<span className="rounded px-1.5 py-0.5 text-[10px] font-semibold text-subtle shadow-[var(--shadow-border)]">V1</span>:null}</div><div className="mt-1 text-xs text-subtle">TEST: {c.testN} casos · éxito {pct(vr?.test.success.pct==null?null:vr.test.success.pct*100)} · Δ vs V1 {delta(c.testDeltaVsBaselinePp)} · {c.evidenceLabel}</div></div><div className="text-right font-mono text-xs tabular"><div>{c.extraDecided} EXTRA decididos</div><div>éxito EXTRA {pct(c.extraSuccessPct)}</div></div></div><div className="mt-2 grid grid-cols-5 gap-2 text-[11px] text-subtle"><span>TP1 {vr?.tp1 ?? "—"}</span><span>TP2 {vr?.tp2 ?? "—"}</span><span>SL {vr?.sl ?? "—"}</span><span>EXTRA {c.additionalOpportunities}</span><span>R {rr(c.meanOutcomeRr)}</span></div></div>})}</div>:<div className="px-4 py-4 text-sm text-subtle">Aún no hay un replay automático guardado. El comparador se rellenará con el próximo ciclo de Watch.</div>}
          {latestReplay?<p className="border-t border-border px-4 py-3 text-[11px] leading-relaxed text-subtle">Último replay: {new Date(latestReplay.generatedAt).toLocaleString("es-ES")}. INSUFFICIENT permanece hasta alcanzar 30 casos TEST adicionales decididos.</p>:null}
        </>}
      </div>
