...
export function LabIntegrityPanel() {
  ...
  const v=values(q.data); const r=radar.data;
  return <section className="mt-2 space-y-3" data-lab-integrity>
    ...
    {q.isLoading?<p className="text-sm text-subtle">Leyendo métricas…</p>:q.isError?<p className="text-sm text-sell">No se ha podido leer el laboratorio. No se inventan cifras.</p>:<>
      ...
      <div className="overflow-hidden rounded-[var(--radius-lg)] bg-elevated px-4 py-3 shadow-[var(--shadow-border)]"><div className="flex items-center justify-between gap-3"><h3 className="font-semibold">Radar de oportunidades perdidas</h3><span className="text-xs font-mono text-subtle">Shadow · descriptivo</span></div>{radar.isLoading?<p className="mt-2 text-sm text-subtle">Analizando historial…</p>:radar.isError||!r?<p className="mt-2 text-sm text-subtle">Radar no disponible.</p>:<><div className="mt-2 grid grid-cols-3 gap-2 text-center"><div><div className="text-lg font-semibold">{r.stats.evaluated}</div><div className="text-[11px] text-subtle">evaluados</div></div><div><div className="text-lg font-semibold">{r.stats.missed}</div><div className="text-[11px] text-subtle">≥1R</div></div><div><div className="text-lg font-semibold">{r.stats.favorableRate==null?"—":`${r.stats.favorableRate}%`}</div><div className="text-[11px] text-subtle">tasa</div></div></div>{r.cases.slice(0,5).map(c=><div key={c.episodeId} className="mt-2 rounded-md border border-current/10 px-3 py-2 text-sm"><div className="flex justify-between gap-3"><span className="font-semibold">{c.assetId} · {c.direction.toUpperCase()}</span><span className="font-mono">{c.mfeR==null?"—":`${c.mfeR.toFixed(2)}R`}</span></div><p className="mt-1 text-xs text-subtle">{c.firstTouch?c.firstTouch.toUpperCase():"Sin toque final"}{c.missingForEntry?` · faltaba: ${c.missingForEntry}`:""}{c.highImpact?" · noticia alta":""}</p></div>)}</>}</div>
