    ...
    {q.isLoading?<p className="text-sm text-subtle">Leyendo métricas…</p>:q.isError?<p className="text-sm text-sell">No se ha podido leer el laboratorio. No se inventan cifras.</p>:<>
      ...
      <div className="overflow-hidden rounded-[var(--radius-lg)] bg-elevated shadow-[var(--shadow-border)]" data-shadow-intrabar>
        ...
        {!intrabar?<p className="px-4 py-4 text-sm text-subtle">Aún no hay cinta 1M/5M capturada. El próximo ciclo de Shadow empezará a medirla.</p>:<>
          <div className="grid grid-cols-2 gap-px border-b border-border bg-border">{intrabar.methods.map(m=><div key={m.method} className="bg-elevated px-4 py-3"><div className="text-[11px] uppercase tracking-wide text-subtle">{m.method}</div><div className="mt-1 font-mono text-lg font-semibold">{m.candidates} candidatos</div><div className="mt-1 text-[11px] text-subtle">{m.earlierThanV1} más rápidos que V1 · {m.extra} EXTRA</div></div>)}</div>
          <div className="divide-y divide-border">{intrabar.methods.map(m=><div key={m.method} className="px-4 py-3"><div className="grid grid-cols-4 gap-2 text-[11px] text-subtle"><span>TP1 {m.tp1}</span><span>TP2 {m.tp2}</span><span>SL {m.sl}</span><span>Pend. {m.pending}</span></div><div className="mt-2 grid grid-cols-3 gap-2 text-xs"><span>Decididos <b>{m.decided}</b></span><span>Éxito <b>{pct(m.successPct)}</b></span><span>R medio <b>{rr(m.meanR)}</b></span></div><div className="mt-1 text-[11px] text-subtle">Episodios con cinta: {m.dataEpisodes}</div></div>)}</div>
