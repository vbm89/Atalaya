    ...
    {q.isLoading?<p className="text-sm text-subtle">Leyendo métricas…</p>:q.isError?<p className="text-sm text-sell">No se ha podido leer el laboratorio. No se inventan cifras.</p>:<>
      ...
      <div className="overflow-hidden rounded-[var(--radius-lg)] bg-elevated shadow-[var(--shadow-border)]" data-shadow-comparator>
        ...
        {radar.isLoading?<p className="px-4 py-4 text-sm text-subtle">Calculando comparación…</p>:radar.isError||!shadowRadar?<p className="px-4 py-4 text-sm text-subtle">Comparador no disponible. No se inventan resultados.</p>:<>
          ...
          {ranked.length>0?<div className="divide-y divide-border">{ranked.map((c,i)=><div key={c.variant} className="px-4 py-3"><div className="flex items-center justify-between gap-3"><div className="min-w-0"><div className="flex items-center gap-2 text-sm font-medium"><span className="font-mono text-xs text-subtle">#{i+1}</span><span className="truncate">{c.variant}</span>{c.variant==="BASELINE_V1"?<span className="rounded px-1.5 py-0.5 text-[10px] font-semibold text-subtle shadow-[var(--shadow-border)]">V1</span>:null}</div><div className="mt-1 text-xs text-subtle">TEST: {c.testN} casos · éxito {pct(c.testDeltaVsBaselinePp==null?null:(comparisons.find(x=>x.variant===c.variant)?.testSuccessPct ?? null))} · Δ vs V1 {delta(c.testDeltaVsBaselinePp)} · {c.evidenceLabel}</div></div><div className="text-right font-mono text-xs tabular"><div>{c.extraDecided} EXTRA decididos</div><div>{rr(c.meanOutcomeRr)}</div></div></div><div className="mt-2 grid grid-cols-4 gap-2 text-[11px] text-subtle"><span>TP1 {c.variant==="BASELINE_V1"?"—":c.extraDecided > 0 ? "ver EXTRA" : "—"}</span><span>TP2 —</span><span>SL —</span><span>EXTRA {c.additionalOpportunities}</span></div></div>)}</div>:<div className="px-4 py-4 text-sm text-subtle">Aún no hay un replay automático guardado. El comparador se rellenará con el próximo ciclo de Watch.</div>}
