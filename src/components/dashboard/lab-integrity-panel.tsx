...
export function LabIntegrityPanel() {
  ...
  const radar=useQuery({queryKey:["shadow-radar","v2"],queryFn:()=>getShadowRadar(),staleTime:30_000,retry:0});
  const v=values(q.data);
  const payload:any=radar.data;
  const shadowRadar=payload?.radar;
  const latestReplay=payload?.latestReplay;
  const intrabar=payload?.intrabar;
  const xauFeeds=payload?.xauFeeds;
  const comparisons=latestReplay?.report?.comparisons ?? [];
  const replayVariants=latestReplay?.report?.replay?.variants ?? [];
  const listed=[...comparisons].sort((a:any,b:any)=>{if(a.variant==="BASELINE_V1")return -1;if(b.variant==="BASELINE_V1")return 1;return String(a.variant).localeCompare(String(b.variant));});
  ...
  return <section className="mt-2 space-y-3" data-lab-integrity>
    ...
    {q.isLoading?<p className="text-sm text-subtle">Leyendo métricas…</p>:q.isError?<p className="text-sm text-sell">No se ha podido leer el laboratorio. No se inventan cifras.</p>:<>
      ...
      <div className="overflow-hidden rounded-[var(--radius-lg)] bg-elevated shadow-[var(--shadow-border)]" data-shadow-comparator>
        <div className="border-b border-border px-4 py-3"><div className="flex items-center justify-between gap-3"><h3 className="font-semibold">Comparador Shadow</h3><span className="text-xs font-mono text-subtle">TEST es juez · k={payload?.preregister?.k ?? 0}</span></div><p className="mt-1 text-xs text-subtle">Orden alfabético. TEST no rankea. k cuenta hipótesis pre-registradas, no filas de replay.</p></div>
        {radar.isLoading?<p className="px-4 py-4 text-sm text-subtle">Calculando comparación…</p>:radar.isError||!shadowRadar?<p className="px-4 py-4 text-sm text-subtle">Comparador no disponible. No se inventan resultados.</p>:<>
          ...
          {listed.length>0?<div className="divide-y divide-border">{listed.map((c:any)=>{const r=replayVariants.find((x:any)=>x.variant===c.variant);return <div key={c.variant} className="px-4 py-3"><div className="flex items-center justify-between gap-3"><div className="min-w-0"><div className="flex items-center gap-2 text-sm font-medium"><span className="truncate">{c.variant}</span>{c.variant==="BASELINE_V1"?<span className="rounded px-1.5 py-0.5 text-[10px] font-semibold text-subtle shadow-[var(--shadow-border)]">V1</span>:null}</div><div className="mt-1 text-xs text-subtle">TEST (juez): {c.testN} casos · éxito {pct(r?.test?.success?.pct==null?null:r.test.success.pct*100)} · {c.evidenceLabel}</div></div><div className="text-right font-mono text-xs tabular"><div>{c.extraDecided} EXTRA decididos</div><div>Δ test {c.testDeltaVsBaselinePp==null?LAB_UNAVAILABLE:`${c.testDeltaVsBaselinePp.toFixed(1)} pp`}</div></div></div><div className="mt-2 grid grid-cols-4 gap-2 text-[11px] text-subtle"><span>TP1 {r?.tp1??"—"}</span><span>TP2 {r?.tp2??"—"}</span><span>SL {r?.sl??"—"}</span><span>EXTRA {c.additionalOpportunities}</span></div><div className="mt-1 grid grid-cols-3 gap-2 text-[11px] text-subtle"><span>Decididos {r?.decided??c.extraDecided}</span><span>Éxito {pct(r?.success?.pct==null?null:r.success.pct*100)}</span><span>R medio {rr(r?.meanOutcomeRr)}</span></div><div className="mt-1 grid grid-cols-3 gap-2 text-[11px] text-subtle"><span>path TP1 {r?.path?.reachedTp1??"—"}</span><span>TP1→SL {r?.path?.terminalSlAfterTp1??"—"}</span><span>ambigua {r?.path?.sameBarAmbiguous??"—"}</span></div></div>})}</div>:<div className="px-4 py-4 text-sm text-subtle">Aún no hay un replay automático guardado.</div>}
      ...
      <div className="overflow-hidden rounded-[var(--radius-lg)] bg-elevated shadow-[var(--shadow-border)]"><div className="border-b border-border px-4 py-3"><h3 className="font-semibold">Métodos por activo</h3><p className="mt-1 text-xs text-subtle">Cada mercado se lista por separado. Sin campeón. TEST no rankea. No cambia V1.</p></div><div className="p-4"><ShadowAssetRankingPanel /></div></div>
      ...
      <div className="overflow-hidden rounded-[var(--radius-lg)] bg-elevated shadow-[var(--shadow-border)]"><div className="border-b border-border px-4 py-3"><div className="flex items-center justify-between gap-3"><h3 className="font-semibold">Horizonte de operación</h3><span className="text-xs font-mono text-subtle">Shadow · 15M</span></div><p className="mt-1 text-xs text-subtle">Misma señal, probada con 6 h, 1 día, 2 días y 3 días. No modifica V1.</p></div>{horizons.length===0?<p className="px-4 py-4 text-sm text-subtle">Aún no hay datos suficientes de cinta para comparar horizontes.</p>:<div className="divide-y divide-border">{horizons.map((h:any)=><div key={h.id} className="px-4 py-3"><div className="text-sm font-medium">{h.label}</div><div className="mt-2 space-y-2">{h.variants.map((c:any)=><div key={c.variant} className="grid grid-cols-[1fr_auto] gap-2 text-xs"><div className="truncate"><span className="font-medium">{c.variant}</span><span className="text-subtle"> · {c.candidates} casos · {c.dataComplete} completos</span></div><div className="font-mono text-right">{pct(c.successPct)} · {c.decided} dec.</div></div>)}</div></div>)}</div>}</div>

      <div className="overflow-hidden rounded-[var(--radius-lg)] bg-elevated px-4 py-3 shadow-[var(--shadow-border)]"><div className="flex items-center justify-between gap-3"><h3 className="font-semibold">Radar de oportunidades perdidas</h3><span className="text-xs font-mono text-subtle">Shadow · descriptivo</span></div>{radar.isLoading?<p className="mt-2 text-sm text-subtle">Analizando historial…</p>:radar.isError||!shadowRadar?<p className="mt-2 text-sm text-subtle">Radar no disponible.</p>:<><div className="mt-2 grid grid-cols-3 gap-2 text-center"><div><div className="text-lg font-semibold">{shadowRadar.stats.evaluated}</div><div className="text-[11px] text-subtle">evaluados</div></div><div><div className="text-lg font-semibold">{shadowRadar.stats.missed}</div><div className="text-[11px] text-subtle">≥1R</div></div><div><div className="text-lg font-semibold">{shadowRadar.stats.favorableRate==null?"—":`${shadowRadar.stats.favorableRate}%`}</div><div className="text-[11px] text-subtle">tasa</div></div></div>{shadowRadar.cases.slice(0,5).map((c:any)=><div key={c.episodeId} className="mt-2 rounded-md border border-current/10 px-3 py-2 text-sm"><div className="flex justify-between gap-3"><span className="font-semibold">{c.assetId} · {c.direction.toUpperCase()}</span><span className="font-mono">{c.mfeR==null?"—":`${c.mfeR.toFixed(2)}R`}</span></div><p className="mt-1 text-xs text-subtle">{c.firstTouch?c.firstTouch.toUpperCase():"Sin toque final"}{c.missingForEntry?` · faltaba: ${c.missingForEntry}`:""}{c.highImpact?" · noticia alta":""}</p></div>)}</>}</div>
      <p className="text-[11px] leading-relaxed text-subtle">Replay Shadow se persiste automáticamente. La investigación es descriptiva y aislada: V1 permanece protegida.</p>
    </>}
  </section>;
}
