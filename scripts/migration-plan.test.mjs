...
test("the auth schema ships outside the globbed directory", () => {
  ...
  assert.equal(
    pending.some((p) => p.name === "0001_auth.sql"),
    false,
    "migrations/auth/0001_auth.sql must not be globbed",
  );
  assert.ok(readdirSync(join(migrationsDir, "auth")).includes("0001_auth.sql"));
  assert.ok(
    pending.some((p) => p.name === "0002_watch.sql"),
    "app watch schema is globbed from migrations/*.sql",
  );
});
